...
// 输入框change事件
const hanldeUploadFile = async (uploadFile) => {
    console.log("进入函数内部");

    const file = uploadFile.videoFile;
    // 单个上传文件
    // 设置整个文件的格式
    let inTaskArrItem = reactive({
        id: new Date(), // 因为forEach是同步，所以需要用指定id作为唯一标识
...
const uploadSignleFile = (taskArrItem) => {
    ...
    const uploadChunk = async (needObj) => {
        ...
        console.log('单个切片上传', chunk_id, "返回内容", res);
...
const handleMerge = async (taskArrItem) => {
    const { fileHash, chunk_end_id, videoTitle, videoDescription, videoImage } = taskArrItem
    const fd = new FormData()
    fd.append('upload_id', fileHash)
